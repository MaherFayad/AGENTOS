...
| REQ-SES-61 | §3.1 · `Plan §23.8` | **Expired as designed, and replaced.** The group said it was *unreadable* and named both reasons; `RUNNER_ROUTES.threads` landed and `ops.thread` met a running Postgres, so both are false and the notice is gone. What ships now keeps the distinction the notice existed to protect: loading, unreachable, drifted and a real zero are four separate renders, and no message body reaches the list payload | `apps/web/src/threads/ThreadsView.tsx` · `apps/web/src/threads/lib/list.ts` | `apps/web/src/threads/lib/list.test.ts` |
