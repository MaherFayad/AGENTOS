...
| REQ-DSH-22 | §2.5.5 | Seven canonical widget types **plus at most three named extensions, ever** (ADR-028); unknown `type` renders a bordered placeholder and never crashes | `apps/web/src/dashboards/components/WidgetView.tsx` · `packages/contracts/src/panels.ts` | `apps/web/src/dashboards/__tests__/widgets.test.mjs` |
...
| REQ-DSH-45 | §2.5.5 | **The cap can fail a build.** A fourth extension widget type stops compiling (`WIDGET_TYPE_EXTENSIONS_USED: 0 \| 1 \| 2 \| 3`) *and* fails the panel validator's parity gate, which grades `panels.ts` rather than its own mirror. An extension name outside ADR-028's three is refused even at three (ADR-028, `Plan §23.7`) | `packages/contracts/src/panels.ts` · `scripts/validate-panels.mjs` | `scripts/__tests__/validate-panels.test.mjs` · `apps/web/src/dashboards/__tests__/widgets.test.mjs` |
| REQ-DSH-46 | §2.5.5 | A **reserved** type (`board`, `calendar`) is refused by the validator with a sentence naming ADR-028, stays out of `WidgetType` so `WidgetView`'s `never` fallthrough is not spent on it, and renders the unsupported placeholder | `packages/contracts/src/panels.ts` · `scripts/validate-panels.mjs` · `apps/web/src/dashboards/components/WidgetView.tsx` | `scripts/__tests__/validate-panels.test.mjs` · `apps/web/src/dashboards/__tests__/widgets.test.mjs` |
| REQ-DSH-47 | §2.5.5 | `thread-feed` — the activity plane grouped by `threadId`, newest thread first, group header a truncated id (a thread has no title, `thread-model.md` §9.6). No new `query.source` and no thread id in a panel file (ADR-028) | `apps/web/src/dashboards/components/ThreadFeed.tsx` · `apps/web/src/dashboards/lib/rows.ts` · `apps/web/src/dashboards/data/use-resolved.tsx` | `apps/web/src/dashboards/__tests__/widgets.test.mjs` |
| REQ-DSH-48 | §2.5 | `thread-feed` distinguishes the two emptinesses it can observe: nothing arrived → `emptyState`; rows arrived carrying no thread → `unthreadedState` with the **counted** number. A run with no `threadId` is dropped, never bucketed into a synthetic thread, and `threadId` survives both the resolver and the row normaliser | `apps/web/src/dashboards/components/ThreadFeed.tsx` · `apps/web/src/dashboards/data/resolve.ts` · `scripts/validate-panels.mjs` | `apps/web/src/dashboards/components/ThreadFeed.test.tsx` · `apps/web/src/dashboards/__tests__/widgets.test.mjs` · `scripts/__tests__/validate-panels.test.mjs` |
