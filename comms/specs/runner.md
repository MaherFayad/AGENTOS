...
| REQ-RUN-47 | §3.2 | The message route is `POST /api/p/:project/thread/:id/message` — the project is resolved from the path **before** any row is read, and `Plan §12`'s unscoped spelling is neither mounted nor a legacy path | `packages/contracts/src/api.ts` · `apps/runner/src/routes/api.ts` | `apps/runner/src/routes/__tests__/thread-routes.test.ts` |
| REQ-RUN-48 | §3.2 | The mailbox is drained at **settled tool calls only** (`ok`/`error`, never `start`), scoped by project in the statement, in `seq` order | `apps/runner/src/lib/mailbox.ts` · `apps/runner/src/db/thread-reads.ts` | `apps/runner/src/lib/__tests__/mailbox.test.ts` |
| REQ-RUN-49 | §3.2 | A drain marks `delivered_at` **once** (`AND delivered_at IS NULL`), so "when did the agent first see this" survives a re-drain; an empty mailbox issues no UPDATE | `apps/runner/src/lib/mailbox.ts` | `apps/runner/src/lib/__tests__/mailbox.test.ts` |
| REQ-RUN-50 | §3.2 | A drain that finds a `halt` stops **at** that message (inclusive) and leaves everything behind it in the mailbox for the run that resumes the thread | `apps/runner/src/lib/mailbox.ts` | `apps/runner/src/lib/__tests__/mailbox.test.ts` |
| REQ-RUN-51 | §3.2 | A `steer` is **refused, never downgraded** — `interrupt_not_deliverable` at the route, and if one reaches the mailbox the drain leaves it undelivered rather than consuming it as a note. `MID_RUN_STEER.supported` is typed `false` | `apps/runner/src/lib/mailbox.ts` · `apps/runner/src/lib/threadService.ts` | `apps/runner/src/lib/__tests__/mailbox.test.ts` |
| REQ-RUN-52 | §3.2 | `halt` is stop-checkpoint-ask: the session aborts at the boundary, the work so far is still extracted as the run's artifact, a `question` with a mandatory `expires_at` is appended, and the thread moves `running → waiting`. Brain write-back and delivery are skipped | `apps/runner/src/lib/runService.ts` | `apps/runner/src/lib/__tests__/thread-run.test.ts` |
| REQ-RUN-53 | §3.2 | Continuing a thread starts a **new run seeded with the thread's history** — there is no resume-the-SDK-session path — and the history lands in the *user* turn, capped, never in the system prompt | `apps/runner/src/lib/prompt.ts` · `apps/runner/src/lib/runService.ts` | `apps/runner/src/lib/__tests__/thread-run.test.ts` |
| REQ-RUN-54 | §3.5 | Every run belongs to a thread: `startRun` opens one when the caller supplies none, and `recordRun` **names `ops.agent_runs.thread_id`** with a placeholder count that matches its column list | `apps/runner/src/db/ledger.ts` · `apps/runner/src/lib/runService.ts` | `apps/runner/src/db/__tests__/writer-schema-agreement.test.ts` |
| REQ-RUN-55 | §3.2 | `POST /api/run/:runId/input` is **never built** (ADR-023) — asserted as an absence across both contracts directories and both apps' sources, with the scanner falsified against a planted line | `comms/decisions/ADR-023-thread-unification.md` | `apps/runner/src/lib/__tests__/superseded-run-input.test.ts` |
| REQ-RUN-56 | §3.2 | Address resolution runs against **this project's resolved roster** and never guesses: an unknown name is `address_unresolved`, and `@slug` matching two departments is `address_ambiguous` with both matches listed | `apps/runner/src/lib/threadService.ts` | `apps/runner/src/lib/__tests__/thread-address.test.ts` |
| REQ-RUN-57 | PART V | Fan-out dispatch is refused at one named enforcement point that **has never fired**: `assertRunnable` is the only branch, it is called from the run pipeline, and `FAN_OUT_DISPATCH.allowed` is typed `false` | `apps/runner/src/lib/threadService.ts` | `apps/runner/src/lib/__tests__/thread-address.test.ts` |
| REQ-RUN-58 | PART III | The nine error codes `thread-model.md` §11 proposed are accepted unrenamed with their proposed statuses, plus `thread_store_unavailable` (503) — a runner with no Postgres refuses rather than serving an in-memory thread | `packages/contracts/src/api.ts` · `apps/runner/src/lib/threadService.ts` | `apps/runner/src/routes/__tests__/thread-routes.test.ts` |
| REQ-RUN-59 | PART III | An error thrown with a **declared** `ApiErrorCode` keeps its code and its sentence through `toApiError`; anything else is still an opaque `internal`, because a raw `err.message` may hold a path, a token or a prompt fragment | `apps/runner/src/lib/errors.ts` | `apps/runner/src/routes/__tests__/thread-routes.test.ts` |
