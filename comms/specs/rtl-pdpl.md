...
| REQ-RTL-12 | PART VII | Right to erasure is stated by **tier**, not claimed executable. Corrected 2026-08-18: this row read *"right to erasure is executable"* and it was **false** — no plane has a delete verb — and its verification was **inert**, asserting four unrelated strings and never the claim. COMPANY.md §7 rule 7 now states tier 1 (project) and tier 2 (author) as selectable-but-not-executable, and tier 3 (a third party named inside free text) as **not selectable at any price, which no delete verb fixes** | `company/COMPANY.md` | `scripts/__tests__/rtl-pdpl.test.mjs` |
| REQ-RTL-37 | PART VII | A message body never leaves the process as observability data at **any** granularity, truncation included — no span attribute, no log, no push payload | `packages/contracts/src/threads.ts` (`messageSpanAttributes`) · `apps/runner/src/observability/redaction-rules.ts` | `apps/runner/src/observability/__tests__/message-body-never-traced.test.ts` |
| REQ-RTL-38 | PART VII | The structural defence has a **backstop**: a message object handed to any instrumentation entry point is redacted at the `body` key rather than shipped, and `bodyChars` survives so the sanctioned projection still works | `apps/runner/src/observability/redaction-rules.ts` | `apps/runner/src/observability/__tests__/message-body-never-traced.test.ts` |
| REQ-RTL-39 | PART VII | **Declared, not enforced.** Free text carrying a body under a non-`body` key — an error string, a composed frame — still reaches a span in full. No key rule reaches it and no value rule should try. Asserted as a known gap so it cannot be rediscovered; closing it is a `RunTrace` type change owned by `observability-engineer` | — | `apps/runner/src/observability/__tests__/message-body-never-traced.test.ts` |
| REQ-RTL-40 | PART VII | **Declared, not enforced.** The model is a processor and a thread's history reaches it verbatim (`lib/prompt.ts`); this repo asserts no processing region for the model endpoint. *"Traces stay local"* answers for the observability plane only. Settled in the data-egress ADR, with the human | `company/COMPANY.md` | — |
