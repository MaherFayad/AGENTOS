...
| REQ-DRW-37 | `Plan §12` | The run console is no longer one-way: a mailbox composer appends a turn to the run's thread through `POST /api/p/:project/thread/:id/message`, with the interrupt level declared by the sender | `apps/web/src/drawer/threads/MailboxComposer.tsx` · `apps/web/src/drawer/threads/mailbox.ts` · `apps/web/src/drawer/data/client.ts` · `apps/web/src/drawer/sections/RunConsole.tsx` | `apps/web/src/drawer/threads/MailboxComposer.test.tsx` · `apps/web/src/drawer/threads/mailbox.test.ts` |
| REQ-DRW-38 | `Plan §12` | **M16 offers two levels and refuses the third.** `steer` is presented as refused with a stated reason and is **untypeable** at the call site — `ComposableLevel` excludes it while `STEER_DELIVERY.supported` is `false`, so no code path can build the request that would 409, and there is no downgrade-to-note fallback (`thread-model.md` invariant 7) | `apps/web/src/drawer/threads/mailbox.ts` · `apps/web/src/drawer/threads/MailboxComposer.tsx` | `apps/web/src/drawer/threads/mailbox.test.ts` · `apps/web/src/drawer/threads/MailboxComposer.test.tsx` |
| REQ-DRW-39 | `Plan §12` | The refusal's reason is the **catalogue key the badge announces**, never a composer-voice paraphrase — that sentence has already been wrong once, and one sentence cannot drift from itself | `apps/web/src/drawer/threads/MailboxComposer.tsx` · `apps/web/src/i18n/strings.en.ts` | `apps/web/src/drawer/threads/MailboxComposer.test.tsx` |
| REQ-DRW-40 | `Plan §12` | `queued` and `delivered-to-run` get different sentences and neither says "sent"; `threadState` is rendered in the past tense as the state **as at the append**, and a `halt` carries the extra line saying the move has not happened yet | `apps/web/src/drawer/threads/mailbox.ts` · `apps/web/src/i18n/strings.en.ts` | `apps/web/src/drawer/threads/mailbox.test.ts` · `apps/web/src/drawer/threads/MailboxComposer.test.tsx` |
| REQ-DRW-41 | `Plan §12` | With no thread to address the composer renders **disabled with the reason**, not collapsed — and `RUN_STREAM_CARRIES_THREAD_ID` is pinned against `SseStartData`, so the day the producer lands the consumer is a build failure away | `apps/web/src/drawer/threads/mailbox.ts` · `apps/web/src/drawer/JobDrawer.tsx` | `apps/web/src/drawer/threads/mailbox.test.ts` · `apps/web/src/drawer/threads/MailboxComposer.test.tsx` |
