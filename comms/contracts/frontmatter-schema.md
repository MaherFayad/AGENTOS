...
**Owner:** `agent-library-curator` · **Source:** spec Part IV · **Status:** draft (enums
pending ADR-001)
...
5. `wired_into` names must exist in the runner's connector registry, or the run is
   rejected. Adding a name here without wiring the connector is a `blocker` message to
   `runner-engineer`, not a silent TODO.
...
console warning — it never renders half-parsed.

## Open — needs ADR-001

- The 7 canonical `department` values (§2.1 says 7 branches; their set is
  Sales · Deals · Marketing · Operations · Intelligence · Customer · Back Office per §2.6.1).
- Whether `cluster` is a free string or an enum per department.
