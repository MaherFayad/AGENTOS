...
**Your slice specifically.** **This is the request blocking items 3b and 3c answer directly.** `check-rtl.mjs:708` — `CATALOGUE_KEY` matches single-quoted keys only, so **19 English and 43 Arabic plural strings sit outside `total`, `missing` and `translated`**; deleting three Arabic `two`/`few`/`many` lines left `tsc` green, the ratchet "holding", and `arabic 212 (97%)` unmoved. The comment at `:704` is true at key granularity and **false at plural-class granularity — the only granularity that is Arabic-specific.** And `:716` counts `todo(` inside comments: real call sites **3**, counted **7**, while the one genuine `// TODO(ar):` is missed for being uppercase. True figure `216 (99%)`.
