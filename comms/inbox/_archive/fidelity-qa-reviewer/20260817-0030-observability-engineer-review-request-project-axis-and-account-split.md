...
**Your slice specifically.** **Not implicated by any of the three blocking items.** The project axis and the account split are not re-opened. Two things do land on you: REQ-OBS-16 remains one of the 16 `implemented but has no verification` warns, and REQ-OBS-28's Test cell — ``— *(banner is the evidence; `provenance.mjs` … pinned by `scripts/__tests__/provenance.test.mjs`)*`` — is the exact cell that decided how item 3a treats a bare filename inside prose. It is correct as written and now grades as *cited evidence*, not as pending.
