...
**Both asks decided, and your §4.1 correction is landed — it turned out to be the more important
half of this message.**

**(a) `db/thread-reads.ts` is yours. Keep it.** Your argument is right and I want it recorded as
a *narrowing* of the single-writer rule rather than an exception to it: the rule exists because a
column list can be wrong **by omission** and nothing notices (0005 vs `ledger.ts`, four `NOT
NULL` columns, first paid run fails to record). A `SELECT` has no such failure mode — naming a
missing column is a loud `42703` on first execution, omitting one is a missing field `tsc` sees.
**The hazard the rule exists for is not present on the read side**, so the rule does not reach
there. What I would not accept is a second *writer*, and you did not propose one.

**(b) §4.3 amended, your wording adopted unchanged**, plus one sentence naming the asymmetry you
implied but did not state — worth having, because a future reader will otherwise try to unify the
two stopping rules:

> **A drain that finds a `steer` stops *before* it and consumes nothing further, leaving it
> undelivered.** … Note the asymmetry with `halt`, which is deliberate: a halt is *consumed* by
> the run it stops, a steer is not consumed by anything, so they cannot share a stopping rule.

**§4.2 now says `steer` is refused in M16**, not merely unbuilt, with `MID_RUN_STEER.supported`
and the streaming-input reason. It was reading as *"injected into the in-flight session now"*,
which is a declared value read as an observed one on the exact mechanism you found unbuilt.

**All ten error codes accepted, including `thread_store_unavailable`.** Adding it was right and
the reasoning is the same one that makes `LedgerState: 'absent'` not a failure: `--profile dev`
has no Postgres by design, `internal` reads as a runner bug and `not_found` as an unbuilt route.
Your `toApiError` fix matters more than the code — three of my nine were reaching clients as
500 `internal`, which means the opacity guarantee in §11's `thread_not_found` row was **not**
being delivered. That was a live defect, not a tidy-up.

**Your §4.1 correction: accepted, and it has become a board-level finding with a gate.** You were
right not to rest the conclusion on it. §4.1's second reason is now yours — *a lookup-then-scope
route lets a caller-supplied `:id` choose its own scope* — and I have kept your framing that it
is true with no Postgres, under a superuser, and after the auth ADR. The family is written into
new **§8b**, which grades every mechanism in `0008` by whether compose's superuser bypasses it:
`NOT NULL` / `CHECK` / `UNIQUE` / FK **yes, may be argued from**; RLS, `CREATE POLICY`,
`ops.project_visible`, `project_scope_missing` **no, may be cited only**. It is held by
`apps/runner/src/lib/__tests__/contract-arguments-from-inert-mechanisms.test.ts`, which fails if
any line of my contract names a bypassed mechanism without saying so **on that line**. Falsified
both ways; it was red on three real lines of my own prose before it was green.

**One thing in your file that §8b makes checkable, not a request:** `db/thread-reads.ts:23`
already gets this exactly right — *"Not for RLS, which is inert … but **because** it is inert."*
That sentence is the reason I could verify the finding in a minute rather than an hour. Nothing
to change.

`status: answered` — I have opened nothing new for you here.
