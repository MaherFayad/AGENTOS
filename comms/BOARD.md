...
| 041 | **An eighth department, `product`, and eleven connector names weighed one at a time** — the company gains the roles that design and build the product (UX research, product design, frontend, A-to-Z delivery). Chosen by the user **over reusing `operations`**, knowingly, which makes it a schema change and not a folder: the enum is gated in `validate-frontmatter.mjs`, mirrored in `packages/contracts/src/departments.ts`, `scripts/lib/departments.mjs` and `scripts/validate-panels.mjs`, and MAP/CHART/DASHBOARDS are projections (rule 2), so the eighth branch propagates to every view for free — and wrongly if the taxonomy is wrong. Carries: the slug, its index (**7, appended**, so no existing department changes index), its clusters, the finding that `departments.mjs` **silently fell back to seven** on any other count, and the connector widening — **ten registered, `mobbin` refused**. `writes` is set by the rule *files ⇒ `ungated`, records and messages ⇒ `none`*, which puts `figma`, `google-drive`, `github` and `vercel` on the strictest value in the enum. **No credentials, no MCP server**: all eight new rows are `available: false`, because `infra/compose.yaml` has no MCP service and `apps/runner/src` has no `mcpServers` config — observed 2026-08-21 | `agent-library-curator` | **accepted 2026-08-21** — `ADR-041-product-department-and-connector-vocabulary.md`, landed at `b24a6d5`. Number claimed here at `74aea50` **before the file**, per the rule directly below; 041 was free in `comms/decisions/` at claim time and no unregistered file was found that pass. Code at `400684d` (department + three mirrors), `06e8990` (connectors, both halves, parity pin extended from keys to `writes` and `available`), `3e361f3` (four `draft` agents, `positions.json` rebuilt not seeded). Eleven source gates green, each observed separately 19:59–20:00 AST — the tree was **not still**, `shell-navigation-engineer` landed two commits mid-dispatch |
| 042 | **Six departments for a product house; the seven business ones deleted** — the owner is a product designer, so the enum becomes `product`, `design`, `frontend`, `backend`, `ai`, `intelligence`, ordered as the shape of the work clockwise from twelve o'clock. Supersedes ADR-001's table **in full** (its two structural rules survive: one declaration site, `cluster` as a registry-validated free string) and ADR-041's department half (its connector vocabulary is untouched). Carries: three agent moves (`product-designer`→`design`, `frontend-engineer`→`frontend`, `agent-auditor`→`ai/agent-ops`), **nine agents deleted** with their departments and **four dashboards deleted** because each was scoped to a department that no longer exists, positions **re-seeded** (append-don't-insert cannot apply to a removal — every index moves and 360/6 ≠ 360/8), and `backend` shipping with **zero agents**: REQ-LIB-33 is marked **UNMET** rather than reworded, because a fabricated agent to turn the row green is exactly constraint 9's defect. Two findings: a **fourth, unregistered** department mirror in `dashboards/data/endpoints.test.ts` that had already silently missed ADR-041's `product` (now reads `DEPARTMENT_SLUGS`), and the seeder classifier's **trailing-`\b`-after-a-stem** bug — `\b(accessibilit|…)\b` can never match "accessibility-auditor" — inherited from ADR-001's own table, where `\b(competit|…)\b` never matched "competitive-analyst" either; 4 of 18 upstream names were silently unmatched until a positive control was run | `commandcenter-orchestrator` | **accepted 2026-08-22** — `ADR-042-six-departments-for-a-product-house.md`. Number claimed here before the file, per the rule below. Verified: `validate:frontmatter` 7 valid / 0 excluded across 6 departments, `validate:panels` ok, `graph:build` 6 departments · 46 nodes, `test:runner` 370 pass / 0 fail, `test:web` 100 files pass, `typecheck` + `typecheck:tests` 0 errors, `validate:coverage` clean. **A blind repo-wide slug rename was attempted and reverted** — it corrupted `company-interview`'s COMPANY.md `section` options, rewrote the spec of record and eight contracts, and broke 38 runner tests by renaming values whose meaning was *"a department that does not exist"*. A slug used as a negative fixture is not the same token as a slug used as a department |
