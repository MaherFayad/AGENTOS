...
import { marketingAgents, specExampleAgents } from '../model/__fixtures__/agents';
...
describe('<StatLine>', () => {
  ...
  it('changes when the agent set changes — nothing here is authored', () => {
    const markup = renderToStaticMarkup(<StatLine stats={deriveStats(marketingAgents)} />);
    ...
    const withOneMore = renderToStaticMarkup(
      <StatLine stats={deriveStats([...marketingAgents, marketingAgents[0]])} />,
    );
