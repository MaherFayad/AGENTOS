...
async function checkInbox(roster) {
  ...
  for (const dir of await listDir(inbox)) {
    ...
    for (const entry of await listDir(join(inbox, dir.name))) {
      ...
      if ((fm.status === 'answered' || fm.status === 'closed') && !/^##\s+Answer\s*$/m.test(text))
        fail(`${rel}: status "${fm.status}" but there is no "## Answer" section.`);
    }
  }
  return count;
}
...
async function main() {
  ...
  if (process.argv.includes('--json')) {
    console.log(JSON.stringify({ agents, messages, contracts, decisions, errors, warnings }, null, 2));
  } else {
    console.log('\nComms check');
